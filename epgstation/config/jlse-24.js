...
const crf = 23;
const videoFilter = 'fieldmatch,decimate,setpts=N/(24000/1001)/TB';
...
// logger.error(str);  //オプションを確認するときコメントアウトを外す
//FFmpegオプション生成 ここまで
...
(async () =>
{
  ...
  const jlse_args = ['run', '--rm', '-v', '/home/tk/docker/JoinLogoScpTrialSetLinux/JL:/join_logo_scp_trial/JL', '-v', '/home/tk/docker/JoinLogoScpTrialSetLinux/logo:/join_logo_scp_trial/logo', '-v', '/home/tk/docker/JoinLogoScpTrialSetLinux/result:/join_logo_scp_trial/result', '-v', '/home/tk/docker/JoinLogoScpTrialSetLinux/setting:/join_logo_scp_trial/setting', '-v', '/home/tk/docker/JoinLogoScpTrialSetLinux/src:/join_logo_scp_trial/src', '-v', '/mnt/hdd_01_2TB/epgstation/recorded:/app/recorded', '-v', '/mnt/hdd_01_2TB/epgstation/wait_to_encode:/app/wait_to_encode', 'join_logo_scp_trial', input, '-e', '-o', str, '-r', '-d', output_dir, '-n', output_name];
  logger.error(jlse_args);

  var env = Object.create(process.env);
  env.HOME = '/root';
  logger.error(env);
  const child = spawn('docker', jlse_args, { env: env, detached: true });
    ...
    for (let i = 0; i < strbyline.length; i++)
    {
      ...
      switch (str)
      {
        ...
        default: { //進捗表示に必要ない出力データを流す
          // logger.info(strbyline[i]);
          break;
        }
      }
      percent = now_num / total_num;
      // logger.info(JSON.stringify({ type: 'progress', percent: percent, log: log }));
